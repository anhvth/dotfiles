...
venv_create() {
    ...
    if [ -z "$1" ] || [ -z "$2" ]; then
        echo "Usage: venv_create <python-version> <venv-name>"
        echo "Example: venv_create python3.9 my_project"
        return 1
    fi
    ...
    echo "Creating virtual environment '$venv_name' using Python version '$python_path'..."
    
    # Check if venv already exists
    if [ -d "$oupath" ]; then
        echo "Virtual environment '$venv_name' already exists. Please choose a different name."
        return 1
    fi
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "To activate, run: source $oupath/bin/activate"
    else
...
venv_remove() {
    ...
    if [ -n "$env_path" ]; then
        echo -n "Are you sure you want to remove the virtual environment '$env_path'? [y/N]: "
        read confirmation
...
venv() {
    ...
    if [ -n "$env_path" ]; then
        ...
        echo "Set default virtual environment to: $env_path"
        source "$env_path/bin/activate"
